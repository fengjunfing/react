...
class ComponentsCommodityQuerySearchForm extends Component {
  static propTypes = {
      ...
      <CommonSearchForm {...this.props} loading={this.props.loading || this.props.table.some(t => t.get('loading'))}>
        ...
        <FormItem>
          {getFieldDecorator('merchantName')(
            <Input addonBefore={'商户名称'} />
          )}
        </FormItem>
        <FormItem>
          <Input addonBefore={'审核状态'} className={'inputSelect'} addonAfter=
            {getFieldDecorator('auditStatus')(
              <Select style={{ minWidth: 150 }} dropdownMatchSelectWidth={false} dropdownStyle={{ maxWidth: 200 }}>
                {
                  [['', '所有']].concat(Object.entries(this.props.dictData.get('AUDIT_STATUS').toJS())).map(t => <Option key={t[0]} value={t[0]}>{t[1]}</Option>)
                }
              </Select>
            )}
          />
        ...
        <FormItem>
          <Input addonBefore={'销售状态'} className={'inputSelect'} addonAfter=
            {getFieldDecorator('status')(
              <Select style={{ minWidth: 150 }} dropdownMatchSelectWidth={false} dropdownStyle={{ maxWidth: 200 }}>
                {
                  [['', '所有']].concat(Object.entries(this.props.dictData.get('PRODUCT_STATUS').toJS())).map(t => <Option key={t[0]} value={t[0]}>{t[1]}</Option>)
                }
              </Select>
            )}
          />
...
})(ComponentsCommodityQuerySearchForm);

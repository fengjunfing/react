...
class ComponentsICBCECarouselManagerSearchForm extends Component {
  static propTypes = {
          ...
          <Input addonBefore={'类型'} className={'inputSelect'} addonAfter=
            {getFieldDecorator('refererType')(
              <Select style={{ minWidth: 150 }} dropdownMatchSelectWidth={false} dropdownStyle={{ maxWidth: 200 }}>
                {
                  [['', '所有']].concat(Object.entries(this.props.dictData.get('INDEX_REFETER_TYPE').toJS())).map(t => <Option key={t[0]} value={t[0]}>{t[1]}</Option>)
                }
              </Select>
            )}
          />
...
})(ComponentsICBCECarouselManagerSearchForm);

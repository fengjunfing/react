...
export default class ComponentsHomeActivityManagerTable extends Component {
  static propTypes  = {
  ...
  render() {
    let columns = [{
      ...
      customWidth: 60,
      render: (text, record) => (
        record.cover && <img className={style.icon} alt={record.name} src={record.cover} />
      )
    }, {
      title: '背景图',
      ...
      customWidth: 60,
      render: (text, record) => (
        record.backgroundPicture && <img className={style.icon} alt={record.name} src={record.backgroundPicture} />
      )
    }, {
      title: '状态',
      dataIndex: 'status',

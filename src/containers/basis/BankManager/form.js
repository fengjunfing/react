...
import ThisForm from '@/components/basis/BankManager/form';
import areaDataOneLevel from '@/publicData/areaDataOneLevel';

class ContainersBankManagerForm extends BaseComponent {
  static propTypes = {
    ...
    this.form.validateFields((err, values) => {
      if (!err) {
        ...
        for (var v in formData) {
          ...
          if (v === 'requiredMaterialsList' && formData[v]) {
            formData[v] = JSON.stringify(formData[v]);
          }
        }
        if (formData && formData.belongRegion && formData.belongRegion.length > 0) {
          formData.belongRegionName = formData.belongRegion.map(v => areaDataOneLevel[v]).join(' / ');
...
const mapStateToProps = state => {
  ...
  return {
    ...
    menuTree: data.getIn(['form', 'menuTree']),
  };
};
...
export default connect(mapStateToProps, mapDispatchToProps)(ContainersBankManagerForm);

...
const form = (state = formInitState, action) => {
  ...
  case thunkTypes.queryRoleList.REQUESTTYPE:
    return state
      .set('loading', true);
  case thunkTypes.queryRoleList.SUCCESSTYPE:
    return state
      .set('loading', false)
      .set('roleList', action.response.get('result'));
  case thunkTypes.queryRoleList.FAILURETYPE:
    return state
      .set('loading', false);
  default:
    return state;
